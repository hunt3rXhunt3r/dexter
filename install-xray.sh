#!/bin/bash
# DexterEskalarte XRay Premium Script
# © Github.com/dextereskalarte
...
xray_install() {
    ...
    mkdir -p /root/xray
    cd /root/xray || exit
    wget -N --no-check-certificate https://raw.githubusercontent.com/johndesu090/fhs-install-v2ray/master/vless/xrayinstall.sh
...
xray_update() {
    ...
    if [[ -d /usr/local/etc/xray ]]; then
        ...
        #systemctl start xray
        sleep 1
        bash <(curl -L -s https://raw.githubusercontent.com/johndesu090/fhs-install-v2ray/master/vless/xrayinstall.sh)
        sleep 1
        xray_privilege_escalation
    else
...
xray_conf_add_tls() {
    ...
    wget --no-check-certificate https://raw.githubusercontent.com/johndesu090/fhs-install-v2ray/master/vless/tls/config.json -O config.json
    modify_path
    modify_alterid
    modify_inbound_port
    modify_UUID
...
xray_conf_add_xtls() {
    cd ${xray_conf_dir} || exit
    wget --no-check-certificate https://raw.githubusercontent.com/johndesu090/fhs-install-v2ray/master/vless/xtls/config.json -O config.json
    modify_path
    modify_alterid
    modify_inbound_port
    modify_UUID
...
acme_cron_update() {
    wget -N -P /usr/bin/idleleo-xray --no-check-certificate "https://raw.githubusercontent.com/johndesu090/fhs-install-v2ray/master/tls/ssl_update.sh"
...
update_sh() {
    ol_version=$(curl -L -s https://raw.githubusercontent.com/johndesu090/fhs-install-v2ray/master/install-xray.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
    ...
    if [[ "$shell_version" < "$(sort -rV $version_cmp | head -1)" ]]; then
        ...
        [yY][eE][sS] | [yY])
            ...
            wget -N --no-check-certificate -P ${idleleo_xray_dir} https://raw.githubusercontent.com/johndesu090/fhs-install-v2ray/master/install-xray.sh && chmod +x ${idleleo_xray_dir}/install.sh
